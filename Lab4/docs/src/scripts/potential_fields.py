...
def goal_force( ):
    ...
    vector_to_goal = [goal[0] - robot[0], goal[1] - robot[1]]

    # 2. Normalize the vector to get the direction: unit vector
    magnitude = max(0.001, math.hypot(vector_to_goal[0], vector_to_goal[1]))
    direction_to_goal = [vector_to_goal[0] / magnitude, vector_to_goal[1] / magnitude]


    # 3. Scale the direction by the strength parameter to get the force
    # force_to_goal = [direction_to_goal[0] * strength, direction_to_goal[1] * strength]
    force_to_goal = [direction_to_goal[0] * strength, vector_to_goal[1] * strength]
    #########################
    # PART A : END
    #########################
    # print(force_to_goal)
...
def obstacle_force():  
    ...
    for i in range(len(laser_scan.ranges)):
        ...
        # 1. Compute force vector with magnitude 'strength' away from obstacle
        force_from_obstacle = [strength * math.cos(cur_angle), strength * math.sin(cur_angle)]

        # 2. Add this force vector to the 'force_from_obstacles' vector
        force_from_obstacles = add_forces(force_from_obstacles, force_from_obstacle)
...
def get_pf_magnitude_linear(distance):
    ...
    if distance < distance_threshold:
        return max_strength * (distance_threshold - distance) / distance_threshold
    #########################
    #  PART C : END
    #########################
